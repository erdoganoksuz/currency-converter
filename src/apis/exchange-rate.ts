...
export default class ExchangeRateApi {
    ...
    async getTimeSeries(startDate: string, symbols: string, base: string): Promise<TimeSeriesResponse> {
        const url = `${this.apiPath}/timeseries?start_date=${startDate}&end_date=${this.dateHelper.getCurrentISODate()}&base=${base}&symbols=${symbols}`;
        const res = await fetch(url);
        const { rates } = await res.json() as ExchangeApiResponse<TimeSeriesResponse>;
        return rates;
    }
